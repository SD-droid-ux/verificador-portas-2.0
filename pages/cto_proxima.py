...
def aba_ctos_proximas(df_base):
    ...
    # Verifica se coluna STATUS existe para evitar erro
    if "STATUS" not in df_base.columns or "CTO" not in df_base.columns:
        st.error("Colunas 'STATUS' e/ou 'CTO' ausentes na base.")
        return
    ...
    # Normaliza colunas para maiúsculas para evitar problemas de case
    df_filtrado["CTO"] = df_filtrado["CTO"].astype(str).str.upper()
    df_filtrado["STATUS"] = df_filtrado["STATUS"].astype(str).str.upper()
    ...
    for _, row_sat in df_saturadas.iterrows():
        ...
        for _, row_ok in df_ok.iterrows():
            ...
            if distancia <= 500:
                raio_label = (
                    "100m" if distancia <= 100 else
                    "300m" if distancia <= 300 else
                    "500m"
                )
                resultado.append({
