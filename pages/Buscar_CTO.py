...
# Filtro por cidade
cidades = df["cid_rede"].dropna().unique()
cidade_selecionada = st.selectbox("🌆 Selecione a cidade:", sorted(cidades))

# Busca só é permitida após seleção
if cidade_selecionada:
    df_cidade = df[df["cid_rede"] == cidade_selecionada].copy()
    df_cidade["cto_norm"] = df_cidade["cto"].apply(normalizar)
            ...
            with st.spinner("🔄 Analisando CTOs..."):
                ...
                for cto_input in input_ctos:
                    ...
                    if melhores and melhores[0][1] > 65:
                        ...
                        match_df = df_cidade[df_cidade["cto_norm"] == cto_norm_encontrado].copy()
                ...
                if resultados_finais:
                    ...
                    # Filtro por POP (após o resultado)
                    if "pop" in df_resultado.columns and df_resultado["pop"].notna().any():
                        pops_unicos = df_resultado["pop"].dropna().unique()
                        pop_filtrado = st.selectbox("📍 Filtrar por POP (opcional):", options=["Todos"] + sorted(pops_unicos.tolist()))
                        if pop_filtrado != "Todos":
                            df_resultado = df_resultado[df_resultado["pop"] == pop_filtrado]
